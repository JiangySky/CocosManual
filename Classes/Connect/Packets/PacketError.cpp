...
PacketError * PacketError::createError(int errorId, std::string msg)
{
    PacketError * packet = INSTANCE(PacketError);
    packet->receivedPacketId = PACKET_ERROR;
    packet->errorId = errorId;
    packet->requestErrMsg = msg;
    return packet;
}
