...
#ifndef FREE_SAFE
#define FREE_SAFE(_obj)             if (_obj) { free(_obj); _obj = nullptr; }
#define DELETE_SAFE(_obj)           if (_obj) { delete _obj; _obj = nullptr; }
#define MALLOC(_obj, _type, _count) { \
int mallocSize = (_count * sizeof(_type)) + 1; \
...
#define PACKET(_cName, _pId)    \
_cName * packet = (_cName *)PacketManager::getInstance()->getPacket(_pId); \
packet->packetId = _pId;

#define SEND_PACKET(_packet)    \
if (INSTANCE(ClientConnect)->sendPacket(_packet)) { \
    activeSendThread(); \
} else { \
    AppConnect::client()->connectFail(true); \
}
...
// Server:  0:AppleTW; 1:PP; 2:AppleCN; 3:91; 4:百度; 5:腾讯; 6:华为; ... 10:外测; 11:内测; 12:开发#1; 13:开发#2; ...
// BIZ:     0:AppleTW; 1:PP; 2:AppleCN; 3:91; 4:百度; 5:腾讯; 6:华为; ...
#define TARGET_TO_LS    1
#if TARGET_TO_LS == 0
...
#define TARGET_GS_IP    "192.168.1.93"
...
typedef enum _RequestType {
    ...
    PACKET_GET_HANDLE               = 20012,    // request handle
    PACKET_GET_HANDLE_RES           = 40015,    // return remote handle
    PACKET_MAX                      = 65500,    // upper limit
    PACKET_RECONNECT                = 20004,    // reconnect
    PACKET_RECONNECT_RES            = 40004,    // reconnect response
...
#define CN_FORMAT_PRINTF(formatPos, argPos) __attribute__((__format__(printf, formatPos, argPos)))
#elif defined(__has_attribute)
#if __has_attribute(format)
#define CN_FORMAT_PRINTF(formatPos, argPos) __attribute__((__format__(printf, formatPos, argPos)))
#endif // __has_attribute(format)
#else
#define CN_FORMAT_PRINTF(formatPos, argPos)
#endif

#define CN_MAX_STRING_LENGTH (1024*100)
#define CNLOG(_log)     ConnUtil::log(_log)

class ConnUtil {
    
public:
    static void log(std::string log, bool logToFile = false)
    {
        std::cout << log << std::endl;
    }
    
    static std::string format(const char* format, ...) CN_FORMAT_PRINTF(1, 2)
    {
        std::string ret;
        ...
        char* buf = (char*)malloc(CN_MAX_STRING_LENGTH);
        if (buf != nullptr)
        {
            vsnprintf(buf, CN_MAX_STRING_LENGTH, format, ap);
            ret = buf;
            free(buf);
        }
        va_end(ap);
...
class ConnUIDelegate {
    
public:
    virtual void reconnectError(float time) = 0;
    virtual void readyForReconnect() = 0;
    virtual void connectFail(float time) = 0;
    virtual void lostConnect(bool local) = 0;
    
    virtual bool networkEnable() = 0;
    virtual bool isConnectGS() = 0;
    virtual bool needTipServer() = 0;
    virtual void tipNetDelay(int delay) = 0;
    virtual void flyTip(std::string msg, float delay, int type);
