...
uint SocketOutputStream::write(Packet * packet)
{
    ...
    if (writeSize >= 0) {
        isCloseKey = true;
        std::string log = ConnUtil::format("--> Send message %i", packet->packetId);
        ConnUtil::log(log, AppConnect::client()->logToFile);
    } else {
        std::string log = ConnUtil::format("*** Failed to send message %i", packet->packetId);
        ConnUtil::log(log, AppConnect::client()->logToFile);
    }
    return writeSize;
}
...
void SocketOutputStream::logInfo()
{
    ConnUtil::log(ConnUtil::format("SocketOutputStream: om_Head = %i, om_Tail = %i", om_Head, om_Tail));
}
